...
class AccBw extends Component {
    ...
    compute = () => {
        ...
        if(n===1){
            output = ((3*code.evaluate(scope))-(4*code.evaluate(scope1))+code.evaluate(scope2))/(2*h)
        } 
        if(n===2){
            output = ((2*code.evaluate(scope))-(5*code.evaluate(scope1))+(4*code.evaluate(scope2))-code.evaluate(scope3))/pow(h,2)
        } 
