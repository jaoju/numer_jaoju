...
class FirstBw extends Component {
    state = {
        result:[],
        equation: "",
        x: 0,
    ...
    compute = () => {
        ...
        if(n===1){
            output = code.evaluate(scope)-code.evaluate(scope1)
        } 
        if(n===2){
            output = code.evaluate(scope)-(2*code.evaluate(scope1))+code.evaluate(scope2)
        } 
        if(n===3){
            output = code.evaluate(scope)-(3*code.evaluate(scope1))+(3*code.evaluate(scope2))-code.evaluate(scope3)
        } 
        if(n===4){
            output = (code.evaluate(scope)-(4*(code.evaluate(scope1)))+(6*(code.evaluate(scope2)))-(4*(code.evaluate(scope3)))+code.evaluate(scope4))
        } 
